...
export const ANSWER_INSTRUCTION =
  'You are given the question and the query result rows as JSON. Write a concise answer for a resort manager: one or two sentences, lead with the key insight, plain English, no jargon. Use the EXACT numbers from the rows — never round, truncate, or invent figures; if you cite a count, copy it verbatim from the data. Then choose a visualization: for a basic ranking or trend use the simple "chart" hint; for anything richer (grouped or stacked bars, multiple series, a distribution, a heatmap) instead provide a Vega-Lite v5 spec in "vegaLite" — give only "mark" and "encoding" (plus an optional short "title"), referencing the row column names, and DO NOT include a "data" field (the rows are injected automatically). Respond as JSON: {"answer":"...","chart":{"type":"bar|line|none","x":"<column>","y":"<column>"},"vegaLite":<spec or null>}.'
