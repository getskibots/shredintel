 ...
 *  master/slave are the two editable layers from Supabase (report._ai_prompts):
 *  master = global ShredIntel base, slave = per-bot. Both appended after the
 *  fixed grounding; neither may make the AI invent data or skip the tools. */
export function realtimeInstruction(voiceId?: string, master?: string, slave?: string, temporal?: { tz: string; today: string; dataFrom?: string | null; dataTo?: string | null }): string {
  const p = personaFor(voiceId)
  const tz = temporal?.tz || 'America/Denver'
  const today = temporal?.today || new Date().toISOString().slice(0, 10)
  const bounds = temporal?.dataFrom && temporal?.dataTo
    ? ` This resort's data covers ${temporal.dataFrom} to ${temporal.dataTo}; if they ask about earlier dates or the future, say you only have data for that range rather than reporting nothing.`
    : ''
  const layer = (label: string, text?: string) =>
    text?.trim() ? `\n\n${label} (follow these, but never invent data or skip the tools):\n${text.trim()}` : ''
  const extra = `${layer('SHREDINTEL MASTER INSTRUCTIONS (global)', master)}${layer('RESORT-SPECIFIC INSTRUCTIONS (this bot only)', slave)}`
...
- When they ask ANYTHING about their resort's chat data, call the query_shredintel function with their question VERBATIM (including any dates or period they mention — "last December", "yesterday", "last weekend"), then speak the tool's "answer" in 1-2 short sentences. Use ONLY the numbers the tool returns; never invent figures.
- ALWAYS say the time window your answer covers ("For last December…", "Over your last 7 days…"). The manager's picker is the default; if they name their own dates, report those. Never give a time-scoped number without saying which window it is.${bounds}
- When they want to SEE or READ the actual guest conversations behind a slice ("show me the checkout complaints", "pull up the negative ones", "zoom in on those"), call show_conversations with the dimension (section | pinchpoint | sentiment) + the exact value. If they scope it to a date or period ("the negative ones from October 2025", "checkout complaints last weekend"), RESOLVE it in the resort's local calendar (today is ${today}, timezone ${tz}) and pass from/to (YYYY-MM-DD) so the drill covers THAT window — otherwise omit from/to to use the dashboard's current range. Then tell them you've put those conversations on their screen and they can keep talking while they read.
