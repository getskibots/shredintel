...
// --backfill: re-classify already-enriched rows to fill the NEW fields (resolution/
// revenue/language + flavor). Bot list = bots with rows still missing them.
...
let BOTS
if (BACKFILL) {
  // bots that have already-enriched rows still missing the new fields
  BOTS = (await c.query(`select bot_id, count(*)::int n from report.conversation_intel
    where resolution is null group by bot_id order by n desc`)).rows.map((r) => Number(r.bot_id))
  await c.end()
} else {
...
console.log(`Fleet ${BACKFILL ? 'BACKFILL (fill new fields)' : 'enrichment'} · ${BOTS.length} bots${BACKFILL ? '' : ` with pending work${MIN ? ` (min ${MIN})` : ''}`} · biggest first.`)
...
for (const id of BOTS) {
  ...
  const r = spawnSync(process.execPath, [join(DIR, 'enrich.mjs'), String(id), ...(BACKFILL ? ['--backfill'] : [])], { cwd: DIR, stdio: 'inherit' })
