...
export function BotSelector() {
  ...
  const active: BotOption | null = (() => {
    for (const b of bots) {
      ...
      if (location.pathname === `/bot/${b.botId}`) return b
    }
    // /bot/:botId with a bot not in the list
    const match = location.pathname.match(/^\/bot\/(\d+)$/)
    if (match) {
      const id = Number(match[1])
      return { botId: id, label: `Bot ${id}`, route: `/bot/${id}` }
    }
    return null
  })()
