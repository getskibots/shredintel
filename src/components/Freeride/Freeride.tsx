...
import { Loader2, ArrowUp, Sparkles, X } from 'lucide-react'
import { resolveSelection, formatRangeHuman } from '../../lib/period'

/**
 * FREERIDE — the conversational, spinnable data explorer.
 *
 * Phase 1: ask a question, the AI picks a cube slice (never SQL), the engine runs
 * it, a chart draws. Phase 2 (the board): every question adds a chart, so you
 * build up a set through conversation. Each chart spins in place independently —
 * its follow-up chips morph THAT chart to a new slice — and every chart is
 * captioned with its exact definition, so the number is always fact-checkable.
 */
...
interface BoardChart extends SpinResult { id: string }
...
/** One chart on the board — draws the slice + its own follow-up chips (which spin it in place). */
function ChartCard({ chart, busy, onSpin, onRemove }: {
  chart: BoardChart
  busy: boolean
  onSpin: (q: string) => void
  onRemove: () => void
}) {
  const fmtV = (v: number) => fmt(v, chart.spec.measure.format)
  const data = chart.rows.slice(0, 15).map((r) => ({
    name: chart.spec.dims.map((d) => String(r[d.key] ?? '')).join(' / '),
    value: Number(r.value),
  }))
  const isLine = chart.chartType === 'line'
  const height = isLine ? 300 : Math.max(220, data.length * 34 + 40)
  ...
  return (
    <div className="relative rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <button
        ...
        className="absolute right-3 top-3 rounded-lg p-1 text-slate-300 transition hover:bg-slate-100 hover:text-slate-500"
      >
        <X className="h-4 w-4" />
      </button>
      {busy && (
      ...
      {chart.answer && <p className="pr-6 text-[15px] font-medium text-slate-800">{chart.answer}</p>}
      <p className="mt-1 text-[11px] font-medium uppercase tracking-wider text-slate-400">{caption(chart.spec)}</p>
...
export function Freeride() {
  ...
  const [board, setBoard] = useState<BoardChart[]>([])
  const [loading, setLoading] = useState<string | null>(null) // 'new' | chart id | null
  const [error, setError] = useState<string | null>(null)
  const idRef = useRef(0)

  /** replaceId set = spin that chart in place; unset = add a new chart to the top. */
  async function spin(q: string, replaceId?: string) {
    const query = q.trim()
    if (!query || !botId || loading) return
    setLoading(replaceId ?? 'new')
    setError(null)
    const current = replaceId ? board.find((c) => c.id === replaceId)?.spec : undefined
    try {
      const res = await fetch('/api/spin', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ botId: Number(botId), from: range.from, to: range.to, question: query, current }),
      })
      const json = await res.json()
      if (!res.ok) throw new Error(json.error || 'spin failed')
      const chart: BoardChart = { ...(json as SpinResult), id: replaceId ?? String(++idRef.current) }
      setBoard((prev) => (replaceId ? prev.map((c) => (c.id === replaceId ? chart : c)) : [chart, ...prev]))
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      <header className="mb-5">
        ...
        <p className="mt-1 text-sm text-slate-500">Ask anything about this resort's conversations. Each question adds a chart you can spin.</p>
      </header>

      <form
        onSubmit={(e) => { e.preventDefault(); spin(question) }}
        className="sticky top-2 z-20 flex items-center gap-2 rounded-2xl border border-slate-200 bg-white p-1.5 shadow-sm focus-within:border-botscrew-400 focus-within:ring-2 focus-within:ring-botscrew-100"
      >
        <input
          value={question}
        ...
        <button
          type="submit"
          disabled={loading === 'new' || !question.trim()}
          aria-label="Ask"
        ...
        >
          {loading === 'new' ? <Loader2 className="h-4 w-4 animate-spin" /> : <ArrowUp className="h-4 w-4" strokeWidth={2.5} />}
        </button>
      </form>

      {board.length === 0 && loading !== 'new' && (
      ...
      <div className="mt-5 space-y-4">
        {board.map((c) => (
          <ChartCard
            ...
            chart={c}
            busy={loading === c.id}
            onSpin={(f) => spin(f, c.id)}
            onRemove={() => setBoard((prev) => prev.filter((x) => x.id !== c.id))}
          />
        ))}
      </div>
    </div>
